...
const ShippingPolicy = () => {
  ...
  const regions = [
    { name: "Nội thành HN & HCM", time: "Giao trong ngày", cost: "15k - 25k" },
      ...
      <div className="max-w-7xl mx-auto px-6 mt-16">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-16">
          {shippingMethods.map((method, i) => (
            <div key={i} className={`bg-white p-10 rounded-3xl border ${method.color} shadow-lg hover:shadow-2xl transition-all duration-300 transform hover:scale-105`}>
              ...
              <p className="text-xl text-red-600 font-bold mb-4">{method.cost} <span className="text-sm text-neutral-500 font-normal"> / Thời gian: {method.time}</span></p>
          ...
          <div className="space-y-12">
            <section className="bg-white p-10 rounded-3xl shadow-sm border border-neutral-100">
              <div className="flex items-center mb-8">
                ...
                <h2 className="text-3xl font-bold text-neutral-950">Ước tính phí & thời gian giao hàng</h2>
              </div>
              <div className="space-y-6">
                {regions.map((region, idx) => (
                  <div key={idx} className="flex justify-between items-center p-6 bg-neutral-50 rounded-2xl border border-neutral-100 hover:bg-red-50 hover:border-red-100">
                    <div>
                      <h4 className="font-bold text-neutral-800 text-lg">{region.name}</h4>
                      <p className="text-neutral-500 text-sm flex items-center"><Clock3 className="w-4 h-4 mr-1"/> {region.time}</p>
                    </div>
          ...
          <div className="space-y-12">
            <section className="bg-white p-10 rounded-3xl shadow-sm border border-neutral-100 relative">
              ...
              <div className="space-y-10 relative">
                ...
                {[
                  { step: "01", label: "Xác nhận & Đóng gói", desc: "Đơn hàng được xác nhận trong 2h & đóng gói cẩn thận." },
                  { step: "02", label: "Giao hàng cho đối tác", desc: "Hàng được bàn giao cho ViettelPost/GHN/J&T." },
                  { step: "03", label: "Giao tới tay bạn", desc: "Bưu tá giao hàng & bạn thực hiện kiểm tra sản phẩm." }
                ].map((item, idx) => (
            ...
            <section className="bg-white p-10 rounded-3xl shadow-sm border border-neutral-100">
                ...
                <ul className="space-y-4 text-sm text-neutral-700 leading-relaxed">
                    <li>• Quý khách vui lòng kiểm tra kỹ hộp hàng & sản phẩm trước khi thanh toán (với COD).</li>
                    <li>• Nếu phát hiện hộp bị rách/vỡ, Quý khách hãy từ chối nhận & báo ngay hotline.</li>
                    <li>• Chúng tôi **chỉ hỗ trợ giao hàng tới chân chung cư/tòa nhà** cho các đơn hàng lớn (>20kg).</li>
                ...
                <div className="mt-8 p-6 bg-red-50 rounded-2xl flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <p className="text-red-700 text-lg font-extrabold">1900 xxxx (Phím 1)</p>
                    </div>
                </div>
            </section>
          </div>
